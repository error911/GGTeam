...
namespace GGTeam.Tools.Audio
{
    ...
    {
        private readonly SavableValue<bool> soundEnabled = new SavableValue<bool>("GGTeam.Audio.AudioController.soundEnabled", true);
        private readonly SavableValue<bool> musicEnabled = new SavableValue<bool>("GGTeam.Audio.AudioController.musicEnabled", true);
        private readonly SavableValue<float> soundVolume = new SavableValue<float>("GGTeam.Audio.AudioController.soundVolume", 1f);
        private readonly SavableValue<float> musicVolume = new SavableValue<float>("GGTeam.Audio.AudioController.musicVolume", 1f);
        ...
        void IAudioPlayer.SetSourcePositionTo(int audioCode, Vector3 destinationPos)
        {
            ...
            if (!data.Is3Dsound)
            {
                Debug.LogError("попытка управлять 2d как 3d sound");
                return;
            }
