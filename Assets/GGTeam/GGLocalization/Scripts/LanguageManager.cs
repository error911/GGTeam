...
public class LanguageManager : MonoBehaviour, ISerializationCallbackReceiver
{
	...
	[Obsolete("Это будет удалено в будущем. Используйте LanguageManager.GetTextValue и LanguageManager.GetAllKeys")]
	...
	[Obsolete("Используйте CurrentlyLoadedCulture")]
	...
	void Awake ()
	{
		...
		else if(instance != this)
		{
			if(VerboseLogging)
			{
				Debug.LogError("Найдены дубликаты LanguageManager! Удалите один из них");
			}
		...
		if(LoadAvailableCultures())
		{
			if(VerboseLogging)
			{
				Debug.Log ("LanguageManager.cs: Проснулся");
			}	
			...
			{
				Debug.LogError("LanguageManager.cs: Нет доступных языков! Используйте GGTools -> GGLocalization, чтобы создать язык");	
			}
		}
		else
		{
			Debug.LogError("LanguageManager.cs: Рабочее пространство локализации не создано! Используйте GGTools -> GGLocalization, чтобы создать один");
		}
	}
	...
	public void ChangeLanguage(string languageCode)
	{
		...
		{
			Debug.LogError("Не удалось загрузить язык: " + languageCode);
			return;
		}
	...
	{	
		...
		{
			Debug.LogError("Не удалось загрузить язык с кодом ISO-639. Данные были нулевыми или пустыми");
			return false;
		}
		var cultureToLoad = GetCultureInfo(languageCode);
		if(cultureToLoad == null)
		{
			Debug.LogError("Не удалось загрузить язык с кодом ISO-639: " + languageCode + ". Невозможно найти соответствующий SmartCultureInfo");
			return false;
		}
	...
	public bool IsCultureSupported(string languageCode)
	{
		if(availableLanguages == null)
		{
			Debug.LogError("LanguageManager не инициализирован должным образом!");
			return false;	
		}
	...
	public List<SmartCultureInfo> GetSupportedLanguages()
	{
		if(availableLanguages == null)
		{
			Debug.LogError("LanguageManager не инициализирован должным образом! Не могу найти доступные языки!");
			return null;	
		}
