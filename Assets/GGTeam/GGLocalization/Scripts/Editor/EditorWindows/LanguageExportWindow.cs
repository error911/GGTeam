...
void OnGUI()
{
	...
	{	
		...
		GUILayout.Label ("Экспорт языка", EditorStyles.boldLabel);
		GUILayout.Label ("Язык для экспорта: " + chosenCulture.englishName + " - " + chosenCulture.languageCode);
...
{
	string folder = EditorUtility.OpenFolderPanel("Выберите папку для сохранения.", "", "");
	if (folder != null && folder != string.Empty)
	{
		...
		{
			Debug.LogError("Неподдерживаемый формат файла! Невозможно экспортировать файл!");
		}
	}
	else{
		Debug.Log("Не удалось экспортировать язык");
	}
}
...
{
	LanguageExportWindow thisWindow = (LanguageExportWindow)EditorWindow.GetWindow<LanguageExportWindow>("Экспорт");
