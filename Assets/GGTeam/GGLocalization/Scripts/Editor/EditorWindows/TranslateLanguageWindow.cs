...
#region GUI Methods
	...
	void DrawSortField(Rect position)
	{
		...
		newPosition.width = fullWindowWidth * 0.99f;
		LanguageSortType chosenSortType = (LanguageSortType)EditorGUI.EnumPopup(newPosition, "Сортировать по", sortType);
	...
	void DrawWatchFile(Rect position)
	{
		...
		newPosition.width = fullWindowWidth * 0.99f;
		string watchName = "Смотреть файл на предмет изменений.";
		if (isWatchingFile)
		{
			GUI.backgroundColor = Color.red;
			watchName = "Просмотр: " + fileBeingWatched;
		}
	...
	void DrawAutoTranslation(Rect position)
	{
		...
		{
			GUI.Label(newPosition, "Инициализация...", EditorStyles.boldLabel);
		}
		else if(!automaticTranslator.IsInitialized)
		{
			...
			{
				GUI.Label(newPosition, "У вас нет сохраненных настроек автоматического переводчика!", EditorStyles.boldLabel);
			}
		}
		else if(automaticTranslator.IsInitialized)
		{
	...
	void DrawNotAvailableAutoTranslationGUI(float fullWindowWidth, Rect newPosition)
	{
		...
		GUI.Label(newPosition, currentCultureInfo.englishName + " недоступен для перевода", EditorStyles.boldLabel);
	...
	void DrawSettingsField(Rect position)
	{
		...
		Rect newPosition = position;
		newPosition.width = fullWindowWidth * 0.99f;
		if(GUI.Button(newPosition, "Копировать все значения из корня"))
		{
